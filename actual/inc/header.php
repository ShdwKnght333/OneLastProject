...
<? $hwm = getsqlrow( "select name from users where id='".$_SESSION['memberid']."'"); ?>
<table width="100%">
...
<? } ?>
			<div class="header">
				<div class="wrap">
				<div class="logo"> <? /* src="/img/page/offers/foodzonedlogo2.png" */ ?>
...
?>
			...
			<div class="find-place dfind-place">
				<div class="wrap m-search">

					<div class="p-ww">

<? if ( $_SERVER['PHP_SELF'] !== "/approve.php" ) { ?>
...
<option value="<?=$row['region'];?>" <? if ( $_SESSION['mycity'] && $_SESSION['myarea'] ) { if ( $_SESSION['mycity'] == $row['region'] ) { echo "selected"; } } ?>><?=$row['region'];?></option>
<?php endwhile; ?>
</select>
...
<option value="<?=$row3['city'];?>" <? if ( $_SESSION['myarea'] == $row3['city'] ) { echo "selected"; } ?>><?=$row3['city'];?></option>
<?php endwhile; ?>
<? } else { ?>
...
<? if ($_REQUEST['service']) { $_SESSION['service'] = $_REQUEST['service']; } ?>
...
<option value="<?=$rss33['site_service'];?>" <? if ( $_SESSION['service'] == $rss33['site_service'] || $totalResults4 == "1" ) { echo "selected"; } ?>><?=strtoupper(getval( "site_services", "name", $rss33['site_service'] ));?></option>
<? } ?>
